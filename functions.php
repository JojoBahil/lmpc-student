...
if(isset($_POST['btn_login'])){
    ...
    if(empty($awardeenum) || empty($password)){
        ...
        // $sql = "SELECT * FROM tbl_lbp_form WHERE award_no ='$awardeenum' AND password = MD5('$password') AND (status = 'Not Finalized' OR status = 'Finalized' OR status = 'Rejected' OR status = 'REQTOUNFINALIZE') AND (tag = 'NEW' OR tag = 'ON-GOING' OR tag = 'AWARDED THROUGH RESIDENCY' OR tag = 'EXTENDED OJT DUE TO PANDEMIC' OR tag = 'ON-GOING LISTAHANAN') AND 
        // (hei_uii = '14001' OR hei_uii = '14002' OR hei_uii = '14003' OR hei_uii = '14040' OR hei_uii = '14031' OR hei_uii = '14004' OR hei_uii = '14005' OR hei_uii = '14036' OR hei_uii = '14053' OR hei_uii = '14037' OR hei_uii = '14039' OR hei_uii = '14010' OR hei_uii = '14042' OR hei_uii = '14028' OR hei_uii = '14013' OR hei_uii = '14043' OR hei_uii = '14032' OR hei_uii = '14050' OR hei_uii = '14063' OR hei_uii = '14016' OR hei_uii = '14047' OR hei_uii = '14020' OR hei_uii = '14022' OR hei_uii = '14051' OR hei_uii = '14035' OR hei_uii = '14026' OR hei_uii = '14006' OR hei_uii = '14027' OR hei_uii = '14008' OR hei_uii = '14012' OR hei_uii = '14014' OR hei_uii = '14034' OR hei_uii = '14066' OR hei_uii = '14065' OR hei_uii = '14017' OR hei_uii = '14019' OR hei_uii = '14007' OR hei_uii = '14009' OR hei_uii = '14029' OR hei_uii = '14015' OR hei_uii = '14058' OR hei_uii = '14021' OR hei_uii = '14062')";
        /* ACCESS FOR SPECIFIC REGION */
        $sql = "SELECT * FROM tbl_lbp_form a INNER JOIN tbl_heis b ON a.hei_uii = b.hei_uii WHERE a.award_no ='$awardeenum' AND a.password = MD5('$password') AND (a.status = 'Not Finalized' OR a.status = 'Finalized' OR a.status = 'Rejected' OR a.status = 'REQTOUNFINALIZE') AND (a.tag = 'NEW' OR a.tag = 'ON-GOING' OR a.tag = 'AWARDED THROUGH RESIDENCY' OR a.tag = 'EXTENDED OJT DUE TO PANDEMIC' OR a.tag = 'ON-GOING LISTAHANAN' OR a.tag = 'FOR TESTING') AND 
        (b.hei_region_nir = '05 - Bicol Region' OR b.hei_region_nir = '06 - Western Visayas' OR b.hei_region_nir = '13 - NCR' OR b.hei_region_nir = '01 - Ilocos Region' OR b.hei_region_nir = '08 - Eastern Visayas' OR b.hei_region_nir = '11 - Davao Region' OR b.hei_region_nir = '03 - Central Luzon' OR b.hei_region_nir = '15 - BARMM' OR b.hei_region_nir = '15A - BARMM' OR b.hei_region_nir = '15B - BARMM' OR b.hei_region_nir = '16 - Caraga' OR b.hei_region_nir = '12 - Soccsksargen' OR b.hei_region_nir = '10 - Northern Mindanao' OR a.award_no = 'TES-11-11-11111-1111-11111')";

        // (b.hei_region_nir = '01 - Ilocos Region' OR b.hei_region_nir = '08 - Eastern Visayas' OR b.hei_region_nir = '11 - Davao Region' OR b.hei_region_nir = '03 - Central Luzon' OR b.hei_region_nir = '15 - BARMM' OR b.hei_region_nir = '15A - BARMM' OR b.hei_region_nir = '15B - BARMM' OR b.hei_region_nir = '16 - Caraga' OR b.hei_region_nir = '12 - Soccsksargen' OR b.hei_region_nir = '10 - Northern Mindanao')";
        ...
        if($resultCheck > 0){
            ...
            $msglogin = "Failed to login? Possible reasons are: <br><br>1. You entered an incorrect credential <br>2. Your school is not scheduled to access the portal today <br>3. Your application was already exported for processing. <br><br> Please follow the UniFAST official page at <a href='https://www.facebook.com/unifastofficial' target='_blank'>www.facebook.com/unifastofficial</a> for more information and to see the latest news and announcements. <br><br>";
...
//         $string_difference = strlen($emboss_name) - 22;
//         $fname_new = substr($fname, 0, -$string_difference);
//         $emboss_name = $fname_new.' '.$mname.' '.$lname; 
//     }
...
if(isset($_POST['btn_export'])){        
    ...
    $sqlQuery = "SELECT * FROM vw_complete_teslbp_data_2019_2020 WHERE award_no = '$GLOBALawardeenumber'";
    $result = mysqli_query($conn, $sqlQuery);
    $resultCheck = mysqli_num_rows($result);
    while ($row = mysqli_fetch_assoc($result)){
    ...
    //stop processing of exporting pdf if not all necessary field are filled out
    if(empty($db_birth_place) || empty($db_present_street) || empty($db_present_barangay) || empty($db_present_city) || empty($db_present_province) || empty($db_present_zip) || empty($db_permanent_barangay) ||
    ...
    $special_code = $GLOBALawardeenumber.' '.date("Ymd-hi");
    QRcode::png($special_code,"qr.png");
    $pdf = new FPDF('p','mm', array(215.9,330.2));
    $pdf->AddPage();
    $pdf->SetMargins(5,0,0);
    ...
    foreach($arr1 as $i => $r){
        if($arr1[$i] == utf8_decode("ñ") || $arr1[$i] == utf8_decode("Ñ")){
            $pdf->Image('../image/letters/Enye.png',$leftPosition,196.4,5,5);
        }
        elseif($arr1[$i] == " "){
            $pdf->Image('../image/letters/space.png',$leftPosition,196.4,5,5);
        }
        else{
            $pdf->Image('../image/letters/'.$arr1[$i].'.png',$leftPosition,196.4,5,5);     
        }
        $leftPosition=$leftPosition+7.65;        
    }
    $pdf->SetFont('Arial', '', 9);
    ...
    $pdf->Image('../image/check.png',82.8,37,3.5,3.5);
    $pdf->Image('../image/check.png',89,42,3.5,3.5);
   // $pdf->Image('../image/check.png',17.8,35.5,6,6);
    //$pdf->Image('../image/check.png',25,39,6,6);
    $pdf->Cell(55 ,15 ,'' ,1,0,'L');
    $pdf->Image('../image/PrepaidCard.png',80,29.6,89,36);
    $pdf->Image('../image/Select.png',110,36,86,9);
    $pdf->Cell(150 ,15 ,'',1,1,'L');
    ...
    $pdf->Image('../image/Line.png',172 ,38.5,34,50);
    $pdf->Image('../image/check.png',193.5,54.5,3.5,3.5);
    //$pdf->Image('../image/check.png',192.5,53,6,6);
    $pdf->Cell(125,3.5,'' ,'L',0, 'C');
    $pdf->Cell(80,3.5,"Cash Card Number/s:" ,'LBR',1, 'L');
    $pdf->Image('../image/Line.png',172 ,42.5,34,50);
    $pdf->SetFont('Arial','B', 8);
    $pdf->Cell(205,4,"ADDITIONAL INFORMATION FOR WALK-INVINDIVIDUAL PURCHASER, AND CARDHOLDER OF INSTITUTIONAL PURCHASER:" ,1,1, 'L');
    
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(153.5,4.27,"Cardholder's Name:" ,'LR',0, 'L');
    $pdf->Cell(51.5,4.27,"Gender:" ,'LR',1, 'L');
    $pdf->SetFont('Arial','', 11);
    $pdf->Cell(153.5,4.27,$studentFullName,'LR',0, 'C');
    $pdf->Cell(51.5,4.27,$gender ,'LR',1, 'C');
    $pdf->Cell(153.5,4.27,"" ,'LBR',0, 'L');
    $pdf->Cell(51.5,4.27,"" ,'LBR',1, 'L');
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(125.3,4.27,"Permanent Address:" ,'LR',0, 'L');
    $pdf->Cell(28.2,4.27,"Zip Code:" ,'LR',0, 'L');
    ...
    $pdf->Cell(41.75,4.27,"" ,'LBR',0, 'L');
    $pdf->Cell(51.5,4.27,"" ,'LBR',1, 'L');
    $pdf->Image('../image/box.png',162,81.5,44,44);
    
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(30,4.27,"Type of ID Presented:" ,'LR',0, 'L');
    $pdf->Cell(35,4.27,"ID Number Presented:" ,'LR',0, 'L');
    $pdf->Cell(24,4.27,"Profession:" ,'LR',0, 'L');
    $pdf->Cell(22.7,4.27,"Source of Fund:" ,'LR',0, 'L');
    $pdf->Cell(41.7,4.27,"TIN:" ,'LR',0, 'L');
    $pdf->Cell(51.6,4.27,"",'LR',1);
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(30,4.27,"$typeofid" ,'LR',0, 'C');
    $pdf->Cell(35,4.27,"$idnumber" ,'LR',0, 'C');
    $pdf->Cell(24,4.27,"$prof" ,'LR',0, 'C');
    $pdf->Cell(22.7,4.27,"$sourceoffund" ,'LR',0, 'C');
    $pdf->Cell(41.7,4.27,"$tinnumber" ,'LR',0, 'C');
    $pdf->Cell(51.6,4.27,"",'LR',1);
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(30,4.27,"" ,'LBR',0, 'L');
    $pdf->Cell(35,4.27,"" ,'LBR',0, 'L');
    ...
    $pdf->Cell(51.6,4.27,"",'LBR',1);
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(111.7,4.27,"Name of Employer/Company/Business/School:" ,'LR',0, 'L');
    $pdf->Cell(41.7,4.27,"Contact Number/s:" ,'LR',0, 'L');
    $pdf->Cell(51.6,4.27,"Gross Salary:" ,'LR',1, 'L');
    $pdf->SetFont('Arial','', 11);
    $pdf->Cell(111.7,4.27,$hei ,'LR',0, 'C');
    $pdf->Cell(41.7,4.27,$mobile ,'LR',0, 'C');
    $pdf->Cell(51.6,4.27,"$grosssalary" ,'LR',1, 'C');
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(111.7,4.27,"" ,'LBR',0, 'L');
    $pdf->Cell(41.7,4.27,"" ,'LBR',0, 'L');
    $pdf->Cell(51.6,4.27,"" ,'LBR',1, 'L');
    
    $pdf->SetFont('Arial','B', 10);
    $pdf->Cell(205,4,"Gift Card Holder's Information" ,1,1, 'C');
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(95,4.27,"Cardholder's Name:" ,'LR',0, 'L');
    $pdf->Cell(50,4.27,"Contact Number/s:" ,'R',0, 'L');
    $pdf->Cell(60,4.27,"Date of Birth:" ,'LR',1, 'L');
    ...
    $pdf->Cell(60,4.27,"" ,'BR',1, 'L');
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(139,4.27,"Permanent Address:" ,'LR',0, 'L');
    $pdf->Cell(33,4.27,"Zip Code:" ,'R',0, 'L');
    $pdf->Cell(33,4.27,"Relationship with:" ,'LR',1, 'L');
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(139,4.27,"$perstreet, $perbrgy",'LR',0, 'C');
    $pdf->Cell(33,4.27,"$perzip" ,'R',0, 'C');
    $pdf->SetFont('Arial','', 9);
    $pdf->Cell(33,4.27,"the Purchaser" ,'R',1, 'L');
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(139,4.27,"$percity, $perprovince" ,'LBR',0, 'C');
    $pdf->Cell(33,4.27,"" ,'BR',0, 'C');
    $pdf->Cell(33,4.27,"" ,'R',1, 'L');
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(139,4.27,"Present Address:" ,'LR',0, 'L');
    $pdf->Cell(33,4.27,"Zip Code:" ,'R',0, 'L');
    $pdf->SetFont('Arial','', 11);
    $pdf->Cell(33,4.27,"N/A" ,'LR',1, 'C');
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(139,4.27,"$prestreet, $prebrgy",'LR',0, 'C');
    $pdf->Cell(33,4.27,"$prezip" ,'R',0, 'C');
    $pdf->SetFont('Arial','', 9);
    $pdf->Cell(33,4.27,"" ,'R',1, 'C');
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(139,4.27,"$precity, $preprovince" ,'LBR',0, 'C');
    $pdf->Cell(33,4.27,"" ,'BR',0, 'L');
    $pdf->Cell(33,4.27,"" ,'BR',1, 'L');
    
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(205,4.27,"Name to Appear on the Card (maximum of 22 characters):" ,'LR',1, 'L');
    $pdf->Image('../image/boxes.png',6,194.5,195,9);
    $pdf->Cell(205,4.27,"" ,'LR',1, 'L');
    $pdf->Cell(205,4.27,"" ,'LBR',1, 'L');
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(102.5,3,"Amount of Fee/Charges to be paid:" ,'LR',0, 'L');
    $pdf->Cell(53.25,3,"Mode of Payment:" ,0,0, 'L');
    $pdf->Cell(49.25,3,"" ,'R',1, 'L');
    ...
    $pdf->Cell(53.25,3,"" ,0,0, 'C');
    $pdf->Cell(49.25,3,"" ,'R',1, 'C');
    $pdf->Cell(102.5,3,"P                                                           ",'LR',0, 'C');
    $pdf->Cell(53.25,3,"" ,0,0, 'C');
    $pdf->Cell(49.25,3,"" ,'R',1, 'C');
    $pdf->Image('../image/Line.png',25,189.5,70,50);
    $pdf->Image('../image/CashDebit.png',129,208,60,6);
    $pdf->Image('../image/check.png',161,205.5,6,6);
    $pdf->Image('../image/Line.png',173,191.7,30,50);
    
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(102.5,3,"(Initial Cost of the Card)",'LR',0, 'C');
    $pdf->Cell(53.25,3,"" ,0,0, 'C');
    $pdf->Cell(49.25,3,"Acct. No." ,'R',1, 'L');        
    $pdf->Cell(102.5,3,"" ,'LBR',0, 'L');
    $pdf->Cell(53.25,3,"" ,'B',0, 'C');
    $pdf->Cell(49.25,3,"" ,'BR',1, 'L');
    ...
    $pdf->Cell(102.5,3,"",'L',0, 'C');
    $pdf->Cell(102.5,3,"" ,'R',1, 'C');
    $pdf->Image('../image/Line.png',28,207,60,50);
    $pdf->Image('../image/Line.png',129,207,60,50);
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(102.5,3,"Signature Over Printed Name",'L',0, 'C');
    $pdf->Cell(102.5,3,"Signature Over Printed Name" ,'R',1, 'C');   
    ...
    $pdf->Cell(68.33,3,"" ,'R',0, 'C');
    $pdf->Cell(68.33,3,"" ,'R',1, 'C');
    $pdf->Image('../image/Line.png',5,228.5,70,50);
    $pdf->Image('../image/Line.png',73,228.5,70,50);
    $pdf->Image('../image/Line.png',141.5,228.5,70,50);
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(68.33,3,"Customer Associate/NAC" ,'LR',0, 'C');
    $pdf->Cell(68.33,3,"BOO/BSO" ,'R',0, 'C');
    $pdf->Cell(68.33,3,"Branch Head" ,'R',1, 'C');  
    ...
    $pdf->SetFont('Arial','', 7);
    $pdf->Cell(205,4,"for Branches without BOO" ,0,1, 'L');
    $pdf->Image('../image/cut.png',-2, 250, 218, 30);
    $pdf->Cell(205,4,"Validation Print (if paid through cash):" ,0,1, 'L');
    ...
    $pdf->Cell(51.25,5,"" ,'R',0, 'L');
    $pdf->Cell(51.25,5,"" ,'R',1, 'L');
    $pdf->Image('../image/Line.png',5,274,52,50);
    $pdf->Image('../image/Line.png',56,274,52,50);
    $pdf->Image('../image/Line.png',107,274,52,50);
    $pdf->Image('../image/Line.png',159,274,52,50);
    $pdf->SetFont('Arial','', 8);
    $pdf->Cell(51.25,4,"Card Custodian" ,'LR',0, 'C');
    $pdf->Cell(51.25,4,"PIN Mailer Custodian" ,'R',0, 'C');
    $pdf->Cell(51.25,4,"Branch Head/BOO/BSO" ,'R',0, 'C');
    ...
    $pdf->Image($db_photo,158.8,85,50.8,50.8); //Student Photo
    $date = str_replace('/', '-', $day);
    $sqlDateExported = "UPDATE tbl_lbp_form SET date_exported = '$date' WHERE award_no = '$GLOBALawardeenumber'";
