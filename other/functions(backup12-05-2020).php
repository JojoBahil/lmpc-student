...
if(isset($_POST['btn_login'])){
    ...
    if(empty($awardeenum) || empty($password)){
        ...
        $sql = "SELECT * FROM vw_complete_teslbp_data_2020_2021 WHERE award_no ='$awardeenum' AND password = MD5('$password') AND ac_year = '$ACADEMIC_YEAR'";
...
if(isset($_POST['btn_change_password'])){
    ...
    */
    ...
    $sql = "SELECT password FROM vw_complete_teslbp_data_2020_2021 WHERE award_no = '$GLOBALawardeenumber' 
...
if(isset($_POST['btn_export'])){ 
    $sqlQuery = "SELECT * FROM vw_complete_teslbp_data_2020_2021 WHERE award_no = '$GLOBALawardeenumber'";
    ...
    if(empty($db_title) || empty($db_marital_status) || empty($db_birth_place) || empty($db_present_street) || 
    ...
    empty($db_id_number) || empty($db_id_type) || empty($db_emboss_name) || 
    ...
    if($db_pickup_at_hei == "1"){
        $sqlHEIServicingBranch = "SELECT branch FROM `tbl_lbp_servicing_branches` WHERE hei_name = '$hei'";
...
//     $studentFullName = $fname." ".$mname." ".$lname;
    ...
    $pdf->Cell(125,4.27,utf8_decode($studentFullName),'L',0, 'C');
    $pdf->SetFont('Arial','', 8);
    ...
    $pdf->SetFont('Arial','', 11);
    $pdf->Cell(153.5,4,utf8_decode($studentFullName),'LR',0, 'C');
    ...
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(125.3,4.27,utf8_decode($perstreet).', '.utf8_decode($perbrgy).',','LR',0, 'C');
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(28.2,4.27,utf8_decode($perzip) ,'LR',0, 'C');
    $pdf->Cell(51.5,4.27,"" ,'LR',1, 'L');
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(125.3,4.27,utf8_decode($percity).', '.utf8_decode($perprovince)  ,'LBR',0, 'C');
    ...
    $pdf->SetFont('Arial','', 11);
    $pdf->Cell(95,4.27,"utf8_decode($studentFullName)",'LR',0, 'C');
    ...
    $pdf->SetFont('Arial','', 10);
    $pdf->Cell(145,4.27,"utf8_decode($perstreet, $perbrgy)",'LR',0, 'C');
...
//ID
   ...
   $id_pdf->Image($db_id_back_photo,20,180,$back_new_height,$back_new_width);
    ...
    $merge->add('../pdf/forms/'.$GLOBALawardeenumber.'_PrepaidRegistrationForm.pdf');
    $merge->add('../pdf/IDs/'.$GLOBALawardeenumber.'_ID.pdf');
    $merge->output('../pdf/Merged/'.$GLOBALawardeenumber.'.pdf','F');
    
    $sqlPDFLocation = "UPDATE tbl_lbp_form SET pdf_attachment = '../pdf/Merged/".$GLOBALawardeenumber.".pdf' WHERE award_no = '$GLOBALawardeenumber'";
